...
/// </summary>
/// <remarks>
/// Données pures, aucune présentation (ADR-0006) : c'est chaque adaptateur qui décide comment
/// rendre ça. Et la lecture se fait sur le snapshot, jamais en interrogeant le système de
/// fichiers — un tableau de bord se rafraîchit souvent.
/// </remarks>
internal sealed class SystemOverviewWidget(IModuleState<SystemSnapshot> state) : IWidgetProvider
{
    public WidgetDescriptor Descriptor { get; } =
        new("system.overview", "Hub", ShowOnChatDashboard: true, Order: 100);
