...
2. Sauvegarde la base (`sqlite3 ... VACUUM INTO`) dans un fichier nommé pour cette tentative
   précise : `pre-update-<depuis>-vers-<vers>-<horodatage>.db`.
3. Arrête le service, repointe `current` vers la nouvelle version, redémarre.
4. Attend `/healthz` jusqu'à 90 s.
5. **Sain** → conserve les 3 dernières versions et les 5 dernières sauvegardes de mise à jour,
   purge le reste.
...
## 7. Ce qui reste hors de cette étape
